...
- 会话 → **文件变更**页签：按轮次展示文件变更列表；展开文件后可在「行级差异」和「高亮对比」两种视图间自由开关与并排对比；高亮对比默认只显示「修改后」，「修改前」可展开，并排时右侧随左侧行级差异高度内部滚动。
...
- 文件变更「高亮对比」支持折叠：默认只展开「修改后」，「修改前」折叠；两节可独立展开/收起，「修改后」排在「修改前」上方。
- 双视图并排时，卡片高度完全由左侧「行级差异」决定，右侧「高亮对比」内部滚动，不再被高亮代码撑高卡片。
